...
void inputDriver(int *frontDistance,int *leftDiistance,int *leftOut,int *rightOut,int *centreOut);
...
init_devices();
  ...
  while (1) {
      ...
      int frontDistance,leftDiistance, leftOut, rightOut, centreOut;

      inputDriver(&frontDistance,&leftOut,&rightOut,&centreOut);

      Cor__acc_step(frontDistance,leftOut,rightOut,centreOut, &out);
...
void inputDriver(int *frontDistance,int *leftDistance,int *leftOut,int *rightOut,int *centreOut) {
  ...
  unsigned char Left_IR = ADC_Conversion(4);
  //unsigned char Sensor = ADC_Conversion();
  //unsigned char Front_IR_Sensor = ADC_Conversion(6);

  print_sensor(2,1,11);	//Prints Value of Front Sharp Sensor
  print_sensor(2,5,4);	//Prints Value of Front IR Sensor
